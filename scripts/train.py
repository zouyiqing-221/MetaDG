...
from model.MetaDG import MetaDG
from model.MetaDGRunner import MetaDGRunner
...
if __name__ == "__main__":
    ...
    parser.add_argument("--d_s", type=int, default=None)
    parser.add_argument("--d_tod", type=int, default=None)
    parser.add_argument("--d_dow", type=int, default=None)
    parser.add_argument("--d_c", type=int, default=None)
    ...
    if model_name == 'MetaDG':
        cfg["model_args"]["device"] = DEVICE

        if args.d_s is not None:
            cfg["model_args"]['node_emb_dim'] = args.d_s
        if args.d_tod is not None:
            cfg["model_args"]['tod_emb_dim'] = args.d_tod
        if args.d_dow is not None:
            cfg["model_args"]['dow_emb_dim'] = args.d_dow
        if args.d_c is not None:
            cfg["model_args"]['continuous_time_emb_dim'] = args.d_c

        model = MetaDG(**cfg['model_args'])
    model.to(DEVICE)
    ...
    runner = MetaDGRunner(cfg, device=DEVICE, scaler=SCALER, log=log)
